...
import { PRINTERS } from '../src/data/printers.ts'
...
// 1b. engine lh 0
{ const e = estimateFdm({ stats, printer: PRINTERS[0], material: mat, settings: DEFAULT_SETTINGS, params: { ...DEFAULT_FDM_PARAMS, layerHeight: 0 } }, t); console.log('engine lh=0 fiyat:', e.perUnit.price.toFixed(2), 'katman', e.layerCount) }
// 5. büyük adet hızlı
{ const t0 = performance.now(); const e = estimateFdm({ stats, printer: PRINTERS[0], material: mat, settings: { ...DEFAULT_SETTINGS, quantity: 1e9 }, params: DEFAULT_FDM_PARAMS }, t); console.log('qty=1e9:', e.quantity, 'adet', e.plates, 'tabla', (performance.now() - t0).toFixed(0), 'ms, toplam', e.total.price.toFixed(0)) }
{ const e = estimateFdm({ stats, printer: PRINTERS[0], material: mat, settings: { ...DEFAULT_SETTINGS, quantity: Infinity }, params: DEFAULT_FDM_PARAMS }, t); console.log('qty=Inf:', e.quantity) }
// planPlates eşdeğerliği: 37 adet, kapasite 25 → 2 tabla (25 + 12)
{ const e = estimateFdm({ stats, printer: PRINTERS[0], material: mat, settings: { ...DEFAULT_SETTINGS, quantity: 37 }, params: DEFAULT_FDM_PARAMS }, t); console.log('qty=37: tabla', e.plates, 'ppp', e.partsPerPlate, 'süre', formatDuration(e.total.printTimeSec, t)) }
// 6. sıfır girişler
{ const e = estimateFdm({ stats, printer: { ...PRINTERS[0], lifetimeHours: 0 }, material: mat, settings: DEFAULT_SETTINGS, params: { ...DEFAULT_FDM_PARAMS, lineWidth: 0 } }, t); console.log('lineWidth=0, ömür=0:', Number.isFinite(e.perUnit.price) ? 'sonlu ' + e.perUnit.price.toFixed(0) : 'HATA ' + e.perUnit.price) }
...
{ const big = new Float32Array([0,0,0, 256,0,0, 256,256,0, 0,0,0, 256,256,0, 0,256,0, 0,0,5, 256,256,5, 256,0,5, 0,0,5, 0,256,5, 256,256,5])
  ...
  const l = plateLayout(bs, PRINTERS[0], 3, 3); const e = estimateFdm({ stats: bs, printer: PRINTERS[0], material: mat, settings: DEFAULT_SETTINGS, params: DEFAULT_FDM_PARAMS }, t)
...
// reçine regresyon
{ const e = estimateResin({ stats, printer: PRINTERS[2], material: MATERIALS.find((m) => m.tech === 'resin')!, settings: { ...DEFAULT_SETTINGS, quantity: 10 }, params: DEFAULT_RESIN_PARAMS }, t); console.log('reçine 10 adet:', formatDuration(e.total.printTimeSec, t), e.perUnit.price.toFixed(0), '₺/adet') }
