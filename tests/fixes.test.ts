...
// 3. Döndürme sırası: analyze R = Rz·Ry·Rx vs three Euler ZYX
{
  ...
  const diff = Math.hypot(v.x - a[0], v.y - a[1], v.z - a[2])
  console.log('rotasyon uyumu (fark):', diff.toExponential(2))
  assert(diff < 1e-6, 'analyze rotasyonu three Euler ZYX ile uyuşmalı')
}
...
for (const scale of [1, 0.1, 0.05]) {
  ...
  assert(Math.abs(a.layers.volume - b.layers.volume) / a.layers.volume < 0.01 && Math.abs(a.layers.volume - a.volume) / a.volume < 0.05, `dilim ölçek ${scale}: sıra bağımsızlığı/analitik sapma`)
...
{ const t0 = performance.now(); const L = sliceLayers(pos, 0, 71, 0); console.log('lh=0:', L.layerCount, 'katman', L.layerHeight, 'mm,', (performance.now() - t0).toFixed(0), 'ms'); assert(L.layerCount > 0 && L.layerHeight > 0 && performance.now() - t0 < 5000, 'lh=0 donmamalı') }
...
{ const e = estimateFdm({ stats, printer: A1, material: mat, settings: { ...DEFAULT_SETTINGS, quantity: Infinity }, params: DEFAULT_FDM_PARAMS }, t); console.log('qty=Inf:', e.quantity); assert(e.quantity === 1 && Number.isFinite(e.total.price), 'qty=Infinity → 1') }
...
// 6. sıfır girişler
{ const e = estimateFdm({ stats, printer: { ...A1, lifetimeHours: 0 }, material: mat, settings: DEFAULT_SETTINGS, params: { ...DEFAULT_FDM_PARAMS, lineWidth: 0 } }, t); console.log('lineWidth=0, ömür=0:', Number.isFinite(e.perUnit.price) ? 'sonlu ' + e.perUnit.price.toFixed(0) : 'HATA ' + e.perUnit.price); assert(Number.isFinite(e.perUnit.price) && e.perUnit.price > 0, 'sıfır girdilerde sonlu fiyat') }
...
assert(formatDuration(7199, t) === '2 sa 0 dk' && formatDuration(3599, t) === '1 sa 0 dk' && formatDuration(29, t) === '0 dk', 'süre yuvarlama')
// reçine regresyon
...
// Sprint 3a: DFM — ayrı kabuklar ve tutarsız sarım
{
  ...
  assert(s2.manifold.components === 2 && e2.warnings.some((w) => /kabuk/.test(w)), 'iki kabuk uyarısı')
  ...
  assert(s3.manifold.inconsistentEdges > 0 && e3.warnings.some((w) => /tutarsız/.test(w)), 'tutarsız sarım uyarısı')
